 ...
 * ALL TEST ALLOCATION SIZES ARE BASED ON THE ASSUMPTION THAT IRAM1 HAS 0x8000 BITS OF SPACE
 */
...
int test0_start(int test_id)
{
    ...
    *p_index = 0;
    ...
    (*p_index)++;
    ret_val = mem2_dump();
    strcpy(g_ae_xtest.msg, "Check mem2_dump return value is two");
    sub_result = (ret_val == 2) ? 1: 0;
    process_sub_result(test_id, *p_index, sub_result);

    //test 0-[11]
    p[6] = mem2_alloc(0x800);
    (*p_index)++;
    strcpy(g_ae_xtest.msg, "Allocating a sixteenth of the RAM (16.666% of the remainder), check not null, not the same as p[0], and therefore p[5], p[4], p[3], p[2] and p[1]");
    sub_result = (p[6] != p[0] && p[6] != NULL) ? 1 : 0;
    process_sub_result(test_id, *p_index, sub_result);
    ...
    p[1] = NULL;
    ret_val = mem2_dump();
    strcpy(g_ae_xtest.msg, "Free p[1], checking mem2_dump return val is three");
    ...
    p[3] = NULL;
    ret_val = mem2_dump();
    strcpy(g_ae_xtest.msg, "Free p[3], checking mem2_dump return val is four");
    ...
    p[4] = NULL;
    ret_val = mem2_dump();
    strcpy(g_ae_xtest.msg, "Free p[4], checking mem2_dump return val is four");
    sub_result = (ret_val == 4) ? 1 : 0;
    process_sub_result(test_id, *p_index, sub_result);
    ...
    p[2] = NULL;
    ret_val = mem2_dump();
    strcpy(g_ae_xtest.msg, "Free p[2], checking mem2_dump return val is three");
    sub_result = (ret_val == 3) ? 1 : 0;
    process_sub_result(test_id, *p_index, sub_result);
    ...
    p[6] = NULL;
    ret_val = mem2_dump();
    strcpy(g_ae_xtest.msg, "Free p[6], checking mem2_dump return val is three");
    sub_result = (ret_val == 3) ? 1 : 0;
    process_sub_result(test_id, *p_index, sub_result);
